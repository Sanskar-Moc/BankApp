...
import com.Bank.BankAccount.customers.CustomerBalanceComparator;
import com.Bank.BankAccount.customers.CustomerIdComparator;
import com.Bank.BankAccount.customers.CustomerNameComparator;
...
//				FileStorageDao.deserializeData(this);
		...
		try {
			Customer.setRefId(JDBCUtils.selectRef());
			Account.setRefAccountNumber(JDBCUtils.selectRef$());
	...
	public void assign(int id,String choice) throws InsufficientBalanceException { // assigns bank account to a Customer instance
		...
		if(choice.equals("SA")) {
			...
			try {
				JDBCUtils.insertAccount(Account.getRefAccountNumber(), bal, currDate.toString());
				JDBCUtils.insertSaving(Account.getRefAccountNumber(), minBal, sal);
				JDBCUtils.updateRecord(id, Account.getRefAccountNumber());
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			...
			try {
				JDBCUtils.insertAccount(Account.getRefAccountNumber(), 0, currDate.toString());
				JDBCUtils.insertFD(Account.getRefAccountNumber(), deposit, ten);
				JDBCUtils.updateRecord(id, Account.getRefAccountNumber());
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
...
//		temp.setAccountAdded(true);
		...
		nameToId.put("*",t );
		try {
			JDBCUtils.updateRefs("*",Account.getRefAccountNumber());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void createNewCustomer(String sex,String name,long mobile,String passport,LocalDate DOB,String email) { // creates new customer and inputs details
		...
		try {
			JDBCUtils.insertCustomer(Customer.getrefId(), name, age[0], mobile, passport, DOB.toString(), email, sex);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		nameToId.put("#",temp);
		try {
			JDBCUtils.updateRefs("#", (long) Customer.getrefId());
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void displayBalAndInterest(int id) { //displays bal and interest of a customer id
		...
		if(temp.accountAdded) {
			double interest=temp.getBankAccount().calculateInterest();
			long acno=temp.getBankAccount().accountNumber;
			String message="Balance: "+temp.getBankAccount().balance
					+"\n"+"Interest: "+interest;
			try {
				if(temp.getBankAccount() instanceof SavingsAccount) {
					JDBCUtils.updateInterest("savingsaccount", interest, acno);		
				}
				else {
					JDBCUtils.updateInterest("fixeddeposit", interest, acno);		
				}
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
